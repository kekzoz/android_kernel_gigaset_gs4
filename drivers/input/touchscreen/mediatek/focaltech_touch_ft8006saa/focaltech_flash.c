    ...
    enum FW_STATUS *fw_sts)
{
    ...
    if (((val[0] == 0x86) && (val[1] == 0x32)) || ((val[0] == 0x86) && (val[1] == 0x42))) {
        FTS_INFO("tp run in romboot");
        *fw_sts = FTS_RUN_IN_ROM;
    } else if (((val[0] == 0x86) && (val[1] == 0xC2)) || ((val[0] == 0x86) && (val[1] == 0xE2))) {
...
static bool fts_fwupg_check_fw_valid(void)
{
    ...
    ret = fts_read_reg(FTS_REG_CHIP_ID2, &val[0]);
    if ((ret >= 0) && (val[0] == 0x32)) {
        msleep(200);
        cmd = FTS_CMD_START1;
        ret = fts_write(&cmd, 1);
        ...
        cmd = FTS_CMD_READ_ID;
        ret = fts_read(&cmd, 1, val, 2);
        FTS_INFO("read boot id:%x %x,ret:%d", val[0], val[1], ret);
        if ((ret >= 0) && (val[0] == 0x86) && (val[1] == 0x32)) {
            FTS_INFO("chip id is 0x8632, boot id, is 0x8632, fw invalid");
            return false;
        }
    }
