...
int idc_esdcheck_lcderror(struct fts_ts_data *ts_data)
{
    ...
    if ( (tp_need_recovery == 1) && (lcd_need_reset == 0) ) {
        ...
        fts8719_tp_state_recovery(ts_data);
    }

    ret = fts8719_read_reg(FTS_REG_ESD_SATURATE, &val);
    if ( ret < 0) {
...
static bool get_chip_id(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (ret < 0) {
            ...
            if ((idh == chip_id) || (fts8719_check_cid(ts_data, idh) == 0)) {
                break;
            } else {
...
static bool get_flow_cnt(struct fts_ts_data *ts_data)
{
    ...
    ret = fts8719_read(&reg_addr, 1, &reg_value, 1);
    if (ret < 0) {
...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
    ...
    ret = fts8719_read_reg(reg_addr, &reg_value);
    if ( ret < 0 ) {
        fts_esdcheck_data.nack_cnt++;
...
static int fts8719_esdcheck_get_status(void)
{
    /* interrupt don't add debug message */
...
/*****************************************************************************
*  Name: fts8719_esdcheck_proc_busy
...
static ssize_t fts8719_esdcheck_store(
    struct device *dev,
...
static ssize_t fts8719_esdcheck_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    count = snprintf(buf, PAGE_SIZE, "Esd check: %s\n", \
                     fts8719_esdcheck_get_status() ? "On" : "Off");
    mutex_unlock(&input_dev->mutex);
 ...
 *
 */
static DEVICE_ATTR (fts_esd_mode, S_IRUGO | S_IWUSR, fts8719_esdcheck_show, fts8719_esdcheck_store);
...
int fts8719_create_esd_sysfs(struct device *dev)
{
    int ret = 0;

    ret = sysfs_create_group(&dev->kobj, &fts_esd_group);
    if ( ret != 0) {
        FTS_ERROR("fts8719_create_esd_sysfs(sysfs) create fail");
...
int fts8719_esdcheck_init(struct fts_ts_data *ts_data)
{
    ...
    fts_esdcheck_data.intr_cnt = 0;
    fts8719_esdcheck_switch(ENABLE);
    fts8719_create_esd_sysfs(ts_data->dev);
    FTS_FUNC_EXIT();
    return 0;
}
