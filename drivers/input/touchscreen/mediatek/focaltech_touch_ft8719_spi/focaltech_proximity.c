...
static int fts_enter_proximity_mode(int mode)
{
    ...
    ret = fts8719_write_reg(buf_addr, buf_value);
    if (ret < 0) {
...
static int ps_enable_nodata(int en)
{
    ...
    /* Enable proximity */
    mutex_lock(&fts8719_data->input_dev->mutex);
    err = fts_enter_proximity_mode(en);
    mutex_unlock(&fts8719_data->input_dev->mutex);
    return err;
}
...
int fts_proximity_readdata(struct fts_ts_data *ts_data)
{
    ...
    fts8719_read_reg(FTS_REG_FACE_DEC_MODE_STATUS, &regvalue);
