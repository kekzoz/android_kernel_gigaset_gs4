...
struct upgrade_module ft8719_module_list[] = {
    {FTS_MODULE_ID, FTS_MODULE_NAME, ft8719_fw_file, sizeof(ft8719_fw_file)},
    {FTS_MODULE2_ID, FTS_MODULE2_NAME, ft8719_fw_file2, sizeof(ft8719_fw_file2)},
    {FTS_MODULE3_ID, FTS_MODULE3_NAME, ft8719_fw_file3, sizeof(ft8719_fw_file3)},
};

struct upgrade_setting_nf ft8719_upgrade_setting_list[] = {
...
struct fts_upgrade *ft8719_fwupgrade;

static int fts8719_check_bootid(void)
{
    int ret = 0;
    u8 cmd = 0;
    u8 id[2] = { 0 };
    struct fts_upgrade *upg = ft8719_fwupgrade;
...
static int fts8719_fwupg_hardware_reset_to_boot(void)
{
    fts8719_reset_proc(0);
    return 0;
}

static int fts8719_enter_into_boot(void)
{
    int ret = 0;
    int i = 0;
    int j = 0;
...
static bool fts8719_check_fast_download(void)
{
    int ret = 0;
    ...
    ret = fts8719_read_reg(0xdb, &value);
    if (ret < 0) {
        FTS_ERROR("read 0xdb fail");
        goto read_err;
    }

    ret = fts8719_read(cmd, 6, value2, 2);
    if (ret < 0) {
...
static int fts8719_dpram_write_pe(u32 saddr, const u8 *buf, u32 len, bool wpram)
{
    int ret = 0;
    int i = 0;
    int j = 0;
    ...
    if (upg->setting_nf->fd_check) {
        fd_support = fts8719_check_fast_download();
        if (!fd_support)
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = fts8719_write(&cmd[0], FTS_CMD_WRITE_LEN + packet_len);
        if (ret < 0) {
...
static int fts8719_dpram_write(u32 saddr, const u8 *buf, u32 len, bool wpram)
{
    int ret = 0;
    int i = 0;
    int j = 0;
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        cmd[2] = BYTE_OFF_8(addr);
        cmd[3] = BYTE_OFF_0(addr);
        ret = fts8719_write(&cmd[0], FTS_ROMBOOT_CMD_SET_PRAM_ADDR_LEN);
        if (ret < 0) {
        ...
        ret = fts8719_write(&cmd[0], 1 + packet_len);
        if (ret < 0) {
            FTS_ERROR("write fw to pram(%d) fail", i);
            goto write_pram_err;
        }
...
static int fts8719_ecc_cal_tp(u32 ecc_saddr, u32 ecc_len, u16 *ecc_value)
{
    int ret = 0;
    int i = 0;
    ...
    ret = fts8719_write(cmd, FTS_ROMBOOT_CMD_ECC_NEW_LEN);
    if (ret < 0) {
    ...
    for (i = 0; i < FTS_ECC_FINISH_TIMEOUT; i++) {
        ret = fts8719_read(cmd, 1, value, 1);
        if (ret < 0) {
    ...
    ret = fts8719_read(cmd, 1, value, 2);
    if (ret < 0) {
...
static int fts8719_ecc_cal_host(const u8 *data, u32 data_len, u16 *ecc_value)
{
    u16 ecc = 0;
    u32 i = 0;
    u32 j = 0;
...
static int fts8719_ecc_check(const u8 *buf, u32 len, u32 ecc_saddr)
{
    int ret = 0;
    int i = 0;
    u16 ecc_in_host = 0;
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = fts8719_ecc_cal_host(buf + offset, packet_length, &ecc_in_host);
        if (ret < 0) {
            FTS_ERROR("ecc in host calc fail");
            return ret;
        }

        ret = fts8719_ecc_cal_tp(ecc_saddr + offset, packet_length, &ecc_in_tp);
        if (ret < 0) {
...
static int fts8719_pram_write_ecc(const u8 *buf, u32 len)
{
    int ret = 0;
    u32 pram_app_size = 0;
    u16 code_len = 0;
    ...
    if (upg->setting_nf->spi_pe)
        ret = fts8719_dpram_write_pe(pram_start_addr, buf, pram_app_size, true);
    else
        ret = fts8719_dpram_write(pram_start_addr, buf, pram_app_size, true);
    if (ret < 0) {
    ...
    /* check ecc */
    ret = fts8719_ecc_check(buf, pram_app_size, pram_start_addr);
    if (ret < 0) {
...
static int fts8719_dram_write_ecc(const u8 *buf, u32 len)
{
    int ret = 0;
    u32 dram_size = 0;
    u32 pram_app_size = 0;
    ...
    /* write pram */
    ret = fts8719_dpram_write(dram_start_addr, dram_buf, dram_size, false);
    if (ret < 0) {
    ...
    /* check ecc */
    ret = fts8719_ecc_check(dram_buf, dram_size, dram_start_addr);
    if (ret < 0) {
...
static int fts8719_pram_start(void)
{
    int ret = 0;
 ...
 */
static int fts8719_fw_write_start(const u8 *buf, u32 len, bool need_reset)
{
    int ret = 0;
    struct fts_upgrade *upg = ft8719_fwupgrade;
    ...
    /* write pram */
    ret = fts8719_pram_write_ecc(buf, len);
    if (ret < 0) {
        FTS_ERROR("write pram fail");
        return ret;
    }
    ...
    if (upg->setting_nf->drwr_support) {
        /* write dram */
        ret = fts8719_dram_write_ecc(buf, len);
        if (ret < 0) {
            FTS_ERROR("write dram fail");
            return ret;
        }
...
static int fts8719_fw_download(const u8 *buf, u32 len, bool need_reset)
{
    int ret = 0;
    int i = 0;
    struct fts_upgrade *upg = ft8719_fwupgrade;
    ...
    fts8719_irq_disable();
#if FTS_ESDCHECK_EN
    fts8719_esdcheck_switch(DISABLE);
#endif

    for (i = 0; i < 3; i++) {
        FTS_INFO("fw download times:%d", i + 1);
        ret = fts8719_fw_write_start(buf, len, need_reset);
        if (0 == ret)
            break;
    }
    if (i >= 3) {
...
#if FTS_ESDCHECK_EN
    fts8719_esdcheck_switch(ENABLE);
#endif
...
static int fts8719_read_file(char *file_name, u8 **file_buf)
{
    int ret = 0;
...
int fts8719_upgrade_bin(char *fw_name, bool force)
{
    int ret = 0;
    u32 fw_file_len = 0;
    u8 *fw_file_buf = NULL;
    ...
    ret = fts8719_read_file(fw_name, &fw_file_buf);
    ...
    ret = fts8719_fw_download(fw_file_buf, fw_file_len, true);
    if (ret < 0) {
...
int fts8719_enter_test_environment(bool test_state)
{
    int ret = 0;
    int i = 0;
    u8 detach_flag = 0;
    ...
    for (i = 0; i < 3; i++) {
        FTS_INFO("fw download times:%d", i + 1);
        ret = fts8719_fw_write_start(upg->fw + app_offset, upg->fw_length, true);
        if (0 == ret)
            break;
    }
    upg->ts_data->fw_loading = 0;
    ...
    msleep(50);
    ret = fts8719_read_reg(FTS_REG_FACTORY_MODE_DETACH_FLAG, &detach_flag);
    FTS_INFO("regb4:0x%02x", detach_flag);

    return 0;
}

int fts8719_fw_resume(bool need_reset)
{
    int ret = 0;
    struct fts_upgrade *upg = ft8719_fwupgrade;
    ...
    ret = fts8719_fw_download(fw_buf, fwlen, need_reset);
    if (ret < 0) {
...
int fts8719_fw_recovery(void)
{
    ...
    ret = fts8719_read_reg(0xD0, &boot_state);
    if (ret < 0) {
    ...
    ret = fts8719_fw_resume(false);
    if (ret < 0) {
        FTS_ERROR("fts8719_fw_resume fail");
        return ret;
    }

    ret = fts8719_read_reg(FTS_REG_CHIP_ID, &chip_id);
    FTS_INFO("read chip id:0x%02x", chip_id);

    fts8719_tp_state_recovery(upg->ts_data);

    FTS_INFO("boot recovery pass");
    return ret;
}

static int fts8719_fwupg_get_module_info(struct fts_upgrade *upg)
{
    int i = 0;
    struct upgrade_module *info = &ft8719_module_list[0];
...
static int fts8719_get_fw_file_via_request_firmware(struct fts_upgrade *upg)
{
    int ret = 0;
    const struct firmware *fw = NULL;
    u8 *tmpbuf = NULL;
...
static int fts8719_get_fw_file_via_i(struct fts_upgrade *upg)
{
    upg->fw = upg->module_info->ft8719_fw_file;
...
/*****************************************************************************
 *  Name: fts8719_fwupg_get_fw_file
 ...
 *****************************************************************************/
static int fts8719_fwupg_get_fw_file(struct fts_upgrade *upg)
{
    int ret = 0;
    ...
    ret = fts8719_fwupg_get_module_info(upg);
    ...
    if (FTS_FW_REQUEST_SUPPORT) {
        msleep(500);
        ret = fts8719_get_fw_file_via_request_firmware(upg);
        if (ret != 0) {
            get_fw_i_flag = true;
        }
    } else {
    ...
    if (get_fw_i_flag) {
        ret = fts8719_get_fw_file_via_i(upg);
    }
...
static void fts8719_fwupg_work(struct work_struct *work)
{
    int ret = 0;
    u8 chip_id = 0;
    struct fts_upgrade *upg = ft8719_fwupgrade;
    ...
    /* get fw */
    ret = fts8719_fwupg_get_fw_file(upg);
    if (ret < 0) {
    ...
    ret = fts8719_fw_download(upg->fw, upg->fw_length, true);
    if (ret < 0) {
...
#ifdef CONFIG_PRIZE_HARDWARE_INFO
		fts8719_read_reg(FTS_REG_FW_VER, &ctp_fw_version);
		sprintf(current_tp_info.id, "Focaltech_FW:0x%02x", ctp_fw_version);
#endif
    }
}

int fts8719_fwupg_init(struct fts_ts_data *ts_data)
{
    int i = 0;
    struct upgrade_setting_nf *setting = &ft8719_upgrade_setting_list[0];
    int setting_count =
        sizeof(ft8719_upgrade_setting_list) / sizeof(ft8719_upgrade_setting_list[0]);
    ...
    ft8719_fwupgrade = (struct fts_upgrade *)kzalloc(sizeof(*ft8719_fwupgrade), GFP_KERNEL);
    if (NULL == ft8719_fwupgrade) {
        FTS_ERROR("malloc memory for upgrade fail");
        return -ENOMEM;
    }

    if (1 == setting_count) {
        ft8719_fwupgrade->setting_nf = setting;
    } else {
        for (i = 0; i < setting_count; i++) {
            setting = &ft8719_upgrade_setting_list[i];
    ...
    if (NULL == ft8719_fwupgrade->setting_nf) {
        FTS_ERROR("no upgrade settings match, can't upgrade");
        kfree(ft8719_fwupgrade);
        ft8719_fwupgrade = NULL;
        return -ENODATA;
    }
    ...
    ft8719_fwupgrade->ts_data = ts_data;
    INIT_WORK(&ts_data->fwupg_work, fts8719_fwupg_work);
    queue_work(ts_data->ts_workqueue, &ts_data->fwupg_work);

    return 0;
}

int fts8719_fwupg_exit(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    if (ft8719_fwupgrade) {
        if (ft8719_fwupgrade->fw_from_request) {
            vfree(ft8719_fwupgrade->fw);
            ft8719_fwupgrade->fw = NULL;
        }
