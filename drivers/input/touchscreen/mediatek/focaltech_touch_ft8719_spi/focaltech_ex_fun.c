    ...
    struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_WRITE_REGISTER:
        ret = fts8719_write_reg(writebuf[1], writebuf[2]);
        if (ret < 0) {
    ...
    case PROC_WRITE_DATA:
        writelen = buflen - 1;
        ret = fts8719_write(writebuf + 1, writelen);
        if (ret < 0) {
    ...
    case PROC_ENTER_TEST_ENVIRONMENT:
        ...
        if (0 == writebuf[1]) {
            fts8719_enter_test_environment(0);
        } else {
            fts8719_enter_test_environment(1);
        }
        break;
    ...
    case PROC_WRITE_DATA_DIRECT:
        writelen = buflen - 1;
        ret = fts8719_spi_transfer_direct(writebuf + 1, writelen, NULL, 0);
        if (ret < 0) {
    ...
    struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
...
#if FTS_ESDCHECK_EN
    fts8719_esdcheck_proc_busy(1);
#endif

    switch (proc->opmode) {
    case PROC_READ_REGISTER:
        num_read_chars = 1;
        ret = fts8719_read_reg(proc->cmd[0], &readbuf[0]);
        if (ret < 0) {
    ...
    case PROC_READ_DATA:
        ...
        ret = fts8719_read(proc->cmd, proc->cmd_len, readbuf, num_read_chars);
        if (ret < 0) {
    ...
    case PROC_READ_DATA_DIRECT:
        num_read_chars = buflen;
        ret = fts8719_spi_transfer_direct(proc->cmd, proc->cmd_len, readbuf, num_read_chars);
        if (ret < 0) {
...
int fts8719_create_apk_debug_channel(struct fts_ts_data *ts_data)
{
    struct ftxxxx_proc *proc = &ts_data->proc;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
...
void fts8719_release_apk_debug_channel(struct fts_ts_data *ts_data)
{
    struct ftxxxx_proc *proc = &ts_data->proc;
...
{
    struct input_dev *input_dev = fts8719_data->input_dev;
    ssize_t count = 0;
...
{
    ssize_t count = 0;
    struct irq_desc *desc = irq_to_desc(fts8719_data->irq);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        ...
        fts8719_data->fw_is_running = false;
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        FTS_INFO("[EX-FUN]set to fw mode");
        fts8719_data->fw_is_running = true;
    }
    mutex_unlock(&input_dev->mutex);
    FTS_FUNC_EXIT();
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    FTS_FUNC_ENTER();
    mutex_lock(&input_dev->mutex);
    if (true == fts8719_data->fw_is_running) {
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts8719_esdcheck_proc_busy(1);
#endif
    ret = fts8719_read_reg(FTS_REG_FW_VER, &fwver);
#if FTS_ESDCHECK_EN
    fts8719_esdcheck_proc_busy(0);
#endif
    ...
    if (rw_op.len < 0) {
        ...
        if (RWREG_OP_READ == rw_op.type) {
            if (rw_op.len == 1) {
                u8 reg, val;
                reg = rw_op.reg & 0xFF;
                rw_op.res = fts8719_read_reg(reg, &val);
                rw_op.val = val;
            } else {
                char reg;
                reg = rw_op.reg & 0xFF;

                rw_op.res = fts8719_read(&reg, 1, rw_op.opbuf, rw_op.len);
            }
            ...
            if (rw_op.len == 1) {
                ...
                reg = rw_op.reg & 0xFF;
                val = rw_op.val & 0xFF;
                rw_op.res = fts8719_write_reg(reg, val);
            } else {
                rw_op.res = fts8719_write(rw_op.opbuf, rw_op.len);
            }
            if (rw_op.res < 0) {
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    for (i = 0; i < fts8719_data->pnt_buf_size; i++) {
        count += snprintf(buf + count, PAGE_SIZE, "%02x ", fts8719_data->point_buf[i]);
    }
    count += snprintf(buf + count, PAGE_SIZE, "\n");
    mutex_unlock(&input_dev->mutex);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&input_dev->mutex);
    sscanf(buf, "%d", &value);
    FTS_DEBUG("log level:%d->%d", fts8719_data->log_level, value);
    fts8719_data->log_level = value;
    mutex_unlock(&input_dev->mutex);
    FTS_FUNC_EXIT();
...
/*  upgrade from fw bin file   example:echo "*.bin" > fts8719_upgrade_bin */
static DEVICE_ATTR(fts8719_upgrade_bin, S_IRUGO | S_IWUSR, fts_fwupgradebin_show, fts_fwupgradebin_store);
...
static struct attribute *fts_attributes[] = {
    ...
    &dev_attr_fts8719_upgrade_bin.attr,
