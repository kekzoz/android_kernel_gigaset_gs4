...
*****************************************************************************/
/* spi interface */
static int fts8719_spi_transfer(u8 *tx_buf, u8 *rx_buf, u32 len)
{
    int ret = 0;
    struct spi_device *spi = fts8719_data->spi;
...
static int fts8719_wait_idle(void)
{
    int ret = 0;
    int i = 0;
    int status = 0xFF;
    ...
    u8 *txbuf = fts8719_data->bus_tx_buf;
    u8 *rxbuf = fts8719_data->bus_rx_buf;
    u32 txlen = 0;

    if (!fts8719_data->fw_is_running)
        idle_status = 0x00;
    ...
    for (i = 0; i < retry_timeout; i++) {
        ...
        ret = fts8719_spi_transfer(txbuf, rxbuf, txlen);
        if (ret >= 0) {
...
static int fts8719_cmdpkg_wirte(u8 ctrl, u8 *cmd, u32 cmdlen)
{
    int i = 0;
    u8 *txbuf = fts8719_data->bus_tx_buf;
    u8 *rxbuf = fts8719_data->bus_rx_buf;
    ...
    if ((!cmd) || (!cmdlen)
        || (cmdlen > (FTS_MAX_COMMMAND_LENGTH - SPI_HEADER_LENGTH))) {
    ...
    return fts8719_spi_transfer(txbuf, rxbuf, txlen);
}

static int fts8719_boot_write(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
    int ret = 0;
    ...
    u8 tmpcmd[FTS_MAX_COMMMAND_LENGTH] = { 0 };
    ...
    ret = fts8719_cmdpkg_wirte(WRITE_CMD, tmpcmd, cmdlen);
    if (ret < 0) {
...
int fts8719_write(u8 *writebuf, u32 writelen)
{
    ...
    return fts8719_boot_write(&cmd[0], cmdlen, data, datalen);
}

int fts8719_write_reg(u8 addr, u8 value)
{
    return fts8719_boot_write(&addr, 1, &value, 1);
}
...
int fts8719_read(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
    ...
    if (cmd && cmdlen) {
        ...
        ret = fts8719_cmdpkg_wirte(ctrl, tmpcmd, cmdlen);
        if (ret < 0) {
            FTS_ERROR("command package wirte fail");
            goto boot_read_err;
        }
...
{
    return fts8719_read(&addr, 1, value, 1);
}
