...
static struct dcam_data_t dcam_r2_devtype = {
	.name = "rear2",
	.is_enabled = 0,
	.sensor_type = 0,
	.pdn_pin = 0,
...
int kdCISModulePowerOn(struct i2c_client* client,  bool On)
{
	...
	//gc2145
		if (On){
		gpio_direction_output(dcam_data->dvdd_pin,1);
        msleep(1);
...
//----------------------------------------------------------------------------
static ssize_t show_camera_rear2_shutter_value(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct i2c_client *client = g_dcam_r2_client;
	struct dcam_data_t *dcam_data = i2c_get_clientdata(client);
	int shutter_value = 0;
...
static ssize_t store_camera_rear2_shutter_value(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	return count;
}
static struct kobj_attribute camera_rear2_value_attr = {
	.attr = {
		 .name = "dcam_r2_value",
		 .mode = 0644,
		 },
	.show = show_camera_rear2_shutter_value,
	.store = store_camera_rear2_shutter_value,
};

static ssize_t show_camera_rear2_open(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct i2c_client *client = g_dcam_r2_client;
	struct dcam_data_t *dcam_data = i2c_get_clientdata(client);
	int ret = -1;
	...
	//mdelay(100);
	mutex_lock(&dcam_r2_mutex);
	if (!dcam_data->is_enabled){
		//kdCISModulePowerOn(client,"camerayuv",true,"kd_camera_hw");
		ret = CameraOpen(client);
		if (ret){
			printk(KERN_ERR"DCAM rear2_open fail %d\n",ret);
			kdCISModulePowerOn(client,false);
			mutex_unlock(&dcam_r2_mutex);
			return sprintf(buf, "%d\n", ret);
		}
		CameraStreamOn(client);
		dcam_data->is_enabled = 1;
	...
	mutex_unlock(&dcam_r2_mutex);
	printk(KERN_INFO"DCAM rear2_open %d\n",dcam_data->is_enabled);
	return sprintf(buf, "%d\n", 0);
}
static ssize_t store_camera_rear2_open(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	return count;
}
static struct kobj_attribute camera_rear2_open_attr = {
	.attr = {
		 .name = "dcam_r2_open",
		 .mode = 0644,
		 },
	.show = show_camera_rear2_open,
	.store = store_camera_rear2_open,
};
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
static ssize_t show_camera_rear2_close(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct i2c_client *client = g_dcam_r2_client;
	struct dcam_data_t *dcam_data = i2c_get_clientdata(client);
	...
	mdelay(100);
	mutex_unlock(&dcam_r2_mutex);
	printk(KERN_INFO"DCAM rear2_close %d\n",dcam_data->is_enabled);
	return sprintf(buf, "%d\n", 0);
}
static ssize_t store_camera_rear2_close(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	return count;
}
static struct kobj_attribute camera_rear2_close_attr = {
	.attr = {
		 .name = "dcam_r2_close",
		 .mode = 0644,
		 },
	.show = show_camera_rear2_close,
	.store = store_camera_rear2_close,
};
//----------------------------------------------------------------------------
...
static const struct of_device_id __maybe_unused dcam_of_match[] = {
	...
	{ .compatible = "prize,dcam_r2", .data = &dcam_r2_devtype },
...
static const struct i2c_device_id dcam_id_table[] = {
	...
	{ "DCAM_R2",(kernel_ulong_t)&dcam_r2_devtype, },
	{ }
};
...
static int dcam_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id){
	...
	if (!strcmp(client->name,"dcam_r")){
	...
	}else if (!strcmp(client->name,"dcam_r2")){
		g_dcam_r2_client = client;
		node = of_find_compatible_node(NULL,NULL,"prize,dcam_r2");
		dcam_data->pdn_pin = of_get_named_gpio(node,"pdn_pin",0);
		dcam_data->rst_pin = of_get_named_gpio(node,"rst_pin",0);
		dcam_data->dvdd_pin = of_get_named_gpio(node,"dvdd_pin",0);
		dcam_data->avdd_pin = of_get_named_gpio(node,"avdd_pin",0);

		printk("DCAM dcam_r2_pdn_pin(%d)  dcam_data->pdn_pin(%d)\n",dcam_data->pdn_pin,dcam_data->pdn_pin);

		gpio_request(dcam_data->pdn_pin,"dcam_r2");
		gpio_direction_output(dcam_data->pdn_pin,0);
		msleep(5); 
		gpio_request(dcam_data->dvdd_pin,"dcam_r2");
		gpio_direction_output(dcam_data->dvdd_pin,1);
        msleep(1);
		gpio_request(dcam_data->avdd_pin,"dcam_r2");
		gpio_direction_output(dcam_data->avdd_pin,1);
        msleep(1); 
        //prize-chj-20190522-Close the back frame after booting according to the timing requirements start
		...
		gpio_direction_output(dcam_data->pdn_pin,1);
        //prize-chj-20190522-Close the back frame after booting according to the timing requirements end
		gpio_request(dcam_data->rst_pin,"dcam_r2");
		gpio_direction_output(dcam_data->rst_pin,0);
		ret = of_property_read_u32(node,"sensor_type",&dcam_data->sensor_type);
		if (ret){
			printk("DCAM dcam_r2 get sensor_type fail ret(%d)\n",ret);
		}

		if (NULL != g_dcam_r2_client){
			ret =  sysfs_create_file(dcam_kobj, &camera_rear2_value_attr.attr);
			if (ret)
				CAMERA_DBG(" kernel sysfs_create_file error \r\n");
			ret =  sysfs_create_file(dcam_kobj, &camera_rear2_open_attr.attr);
			if (ret)
				CAMERA_DBG(" kernel sysfs_create_file error \r\n");
			ret =  sysfs_create_file(dcam_kobj, &camera_rear2_close_attr.attr);
			if (ret)
				CAMERA_DBG(" kernel sysfs_create_file error \r\n");
		}
...
static int __init dcam_init(void){
	...
	if (!dcam_kobj){
		CAMERA_DBG(" kernel kobject_create_and_add dcam error \r\n"); 
		return -1;
	}
...
static void __exit dcam_exit(void){
	...
	if (NULL != g_dcam_r2_client){	   
		sysfs_remove_file(dcam_kobj, &camera_rear2_value_attr.attr);
		sysfs_remove_file(dcam_kobj, &camera_rear2_open_attr.attr);
		sysfs_remove_file(dcam_kobj, &camera_rear2_close_attr.attr);
	}	
	if (dcam_kobj){
