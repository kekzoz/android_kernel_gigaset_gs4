...
#include "gpio.h"
/*prize add by zhuzhengjiang for mipi switch 2(main2 & main3) 20200303 start*/
struct pinctrl		 *ppinctrl_cam;
struct pinctrl_state *ppinctrl_cam_mipi_sel2_h;
struct pinctrl_state *ppinctrl_cam_mipi_sel2_l;
/*prize add by zhuzhengjiang for mipi switch 2(main2 & main3) 20200303 end*/
	...
	struct IMGSENSOR_HW_DEVICE_COMMON *pcommon)
{
	...
	ppinctrl_cam_mipi_sel2_h = pinctrl_lookup_state(pgpio->ppinctrl,"cam_mipi_switch_sel2_1");	
	ppinctrl_cam_mipi_sel2_l = pinctrl_lookup_state(pgpio->ppinctrl,"cam_mipi_switch_sel2_0");
	/*prize add by zhuzhengjiang for mipi switch 2(main2 & main3) 20200303 end*/
	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	/*prize add by zhuzhengjiang for mipi switch 2(main2 & main3) 20200303 start*/
	if(sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN2 && pin == IMGSENSOR_HW_PIN_MCLK && gpio_state ==  IMGSENSOR_HW_PIN_STATE_LEVEL_HIGH)
	{
		if (!IS_ERR(ppinctrl_cam_mipi_sel2_l))
		{
			pinctrl_select_state(ppinctrl_cam, ppinctrl_cam_mipi_sel2_l);
		}
		else
		{
			printk("ppinctrl_cam_mipi2_sel_state h pinctrl err\n");
		}
		mdelay(5);
	}
	else if(sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN3 && pin == IMGSENSOR_HW_PIN_MCLK && gpio_state ==  IMGSENSOR_HW_PIN_STATE_LEVEL_HIGH)
	{
		if (!IS_ERR(ppinctrl_cam_mipi_sel2_h))
		{
			pinctrl_select_state(ppinctrl_cam, ppinctrl_cam_mipi_sel2_h);
		}
		else
		{
