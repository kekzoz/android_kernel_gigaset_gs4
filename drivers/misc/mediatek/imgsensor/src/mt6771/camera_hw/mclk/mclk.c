	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	if (pin_state < IMGSENSOR_HW_PIN_STATE_LEVEL_0 ||
		...
		mutex_unlock(pinst->pmclk_mutex);
		/*prize add by zhuzhengjiang for dcam_r mclk 20200316 start */ 
		if(sensor_idx == 0){
			state_index = (pin_state > IMGSENSOR_HW_PIN_STATE_LEVEL_0) ?
				pinst->drive_current[sensor_idx+2] : MCLK_STATE_DISABLE;

			ppinctrl_state = pinst->ppinctrl_state[sensor_idx+2][state_index];
			...
			if (!IS_ERR(ppinctrl_state))
				...
				PK_PR_ERR(
				"%s : sensor_idx %d pinctrl, PinIdx %d, Val %d, drive current %d\n",
				__func__, sensor_idx+2, pin, pin_state,
				pinst->drive_current[sensor_idx+2]);

			mutex_unlock(pinst->pmclk_mutex);
		}
		/*prize add by zhuzhengjiang for dcam_r mclk 20200316 end */ 
