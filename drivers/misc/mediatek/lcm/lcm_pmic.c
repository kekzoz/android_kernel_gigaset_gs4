...
int display_bias_enable(void)
{
	...
	ret = regulator_set_voltage(disp_bias_pos, 5800000, 5800000);//prize-mod wyq 20190227 set to 5800mV to fix thermal issue
	if (ret < 0)
		pr_info("set voltage disp_bias_pos fail, ret = %d\n", ret);
	retval |= ret;

	ret = regulator_set_voltage(disp_bias_neg, 5800000, 5800000);//prize-mod wyq 20190227 set to 5800mV to fix thermal issue
	if (ret < 0)
