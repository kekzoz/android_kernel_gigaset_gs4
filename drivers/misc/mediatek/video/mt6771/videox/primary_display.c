			 ...
			 int is_lcm_inited)
{
	...
	DISPCHECK("%s: done\n", __func__);
//prize-modify-pengzhipeng-start
...
int primary_display_poweroff_ext(void)
{
	if (pgc->plcm->drv->poweroff_ext){
		pgc->plcm->drv->poweroff_ext();
	}
	return 0;
}
EXPORT_SYMBOL(primary_display_poweroff_ext);
#endif
