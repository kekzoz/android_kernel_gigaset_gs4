...
/* prize modified by lifenfen, fixbug V530A-562, add power mode control,  refer to ALPS04945222, 20200102 begin */
DEFINE_MUTEX(fb_pow_mod_lock);
/* prize modified by lifenfen, fixbug V530A-562, add power mode control,  refer to ALPS04945222, 20200102 end */
...
{
    int i = 0;      /* prize liaoxingen add fixbug 95748, 20200814 */
	MTKFB_FUNC();

    /* prize liaoxingen add fixbug 95748, 20200814 start */
    if(mode == 1){
        while(pgc->state == DISP_SLEPT && i++ < 50){
            msleep(2);
        }
        if(i > 0)
            DDPPR_ERR("%s i=%d,pgc->state=%d\n",__func__,i,pgc->state);
    }
    /* prize liaoxingen add fixbug 95748, 20200814 end */
