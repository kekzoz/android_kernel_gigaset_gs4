...
static ssize_t pin_id_show(struct device *dev, struct device_attribute *attr,
                            char *buf) {
	 struct st21nfc_device *data = dev_get_drvdata(dev);
	 
     return scnprintf(buf, PAGE_SIZE, "%d\n", gpiod_get_value(data->gpiod_id));
} 
...
static int proc_nfc_pin_id_show(struct seq_file *m, void *v)
{
	
	 
	seq_printf(m, "%d\n", gpiod_get_value(st21nfc_dev->gpiod_id));
...
static int proc_nfc_pin_id_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_nfc_pin_id_show, NULL);
}

static const struct file_operations nfc_pin_id_proc_fops = {
	.open = proc_nfc_pin_id_open,
	.read = seq_read,
	.llseek	= seq_lseek,
	.write = proc_write,
};
...
static int st21nfc_probe(struct i2c_client *client,
...
#ifndef ST21NFCD_MTK
  st21nfc_dev->gpiod_pidle = devm_gpiod_get(dev, "id", GPIOD_IN);
#else   // ST21NFCD_MTK
  ret = of_get_named_gpio(np, "gpio-id-std", 0);
  if (gpio_is_valid(ret)) {
    st21nfc_dev->gpiod_id= gpio_to_desc(ret);
  }
#endif  // ST21NFCD_MTK
  ...
  proc_create("nfc_pin_id", 0666, NULL, &nfc_pin_id_proc_fops);
